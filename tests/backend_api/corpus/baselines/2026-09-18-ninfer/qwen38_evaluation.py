...
def prompt_for(store):
    ...
    if name in {"hualian", "華聯", "华联"}:
        common += "\nHualian: right-side price starts a merchandise row. Following rows without a right-side price belong to that item, including bilingual text. Scale weight and unit price appear ABOVE their priced item row. Use printed amounts to cross-check association.\n"
...
def request_for(case, image_root, model, output_mode):
...
def main():
    ...
    for case in snapshot["cases"]:
        request = request_for(case, args.snapshot.parent, args.model, args.output_mode)

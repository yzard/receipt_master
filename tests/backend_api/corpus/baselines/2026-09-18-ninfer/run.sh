...
restore() {
  docker compose -f docker/ninfer_evaluation.compose.yaml logs --no-color > build/ninfer/inference.log || true
...
ready=false
for ((i=0;i<180;i++)); do
  if curl -fsS http://127.0.0.1:5002/health > build/ninfer/health.json; then ready=true; break; fi
  sleep 2
done
...
python3 tests/backend_api/qwen38_evaluation.py \
  ...
  --output build/ninfer/nonthinking \

"""Two text OCR engines and logo embeddings inside one internal container."""
...
def prepare_request(body: dict, config: ServerConfig) -> dict:
    ...
    photo_count = sum(
        1 for message in body["messages"] if isinstance(message, dict)
...
def create_application(config: ServerConfig):
    ...
    async def lifespan(app):
        ...
        paddle = await asyncio.create_subprocess_exec(
            config.paddle.python, str(Path(__file__).with_name("paddle_worker.py")),
            "--port", str(config.paddle.port), "--detection-path", config.paddle.detection_path,
            "--recognition-path", config.paddle.recognition_path, "--threads", str(config.paddle.threads),
        ...
        try:
            async with httpx.AsyncClient(timeout=config.timeout_seconds, trust_env=False) as client:
                ...
                from logo_embedding import LogoEncoder

                app.state.logo_encoder = await asyncio.to_thread(LogoEncoder, config.logo_model_path)
    ...
    @app.post("/v1/logo/embedding")
    async def logo_embedding(body: dict):
        encoded = body.get("image")
        if not isinstance(encoded, str) or len(encoded) > 8 * 1024 * 1024:
            raise HTTPException(400, "Invalid logo image")
        async with app.state.logo_lock:
            try:
                return await asyncio.to_thread(app.state.logo_encoder.encode, encoded)
    ...
    async def completions(body: dict):
        ...
        photos = [part["image_url"]["url"] for message in request["messages"]
        ...
        async with app.state.requests:
            for photo in photos:
                unlimited_request = {
                    ...
                    "temperature": 0, "max_tokens": min(int(body.get("max_tokens", 8192)), config.engine.context_length),
        ...
        usage = {key: sum(u[key] for u in usages) for key in ("prompt_tokens", "completion_tokens", "total_tokens")} if all(
            isinstance(u, dict) and all(isinstance(u.get(k), int) for k in ("prompt_tokens", "completion_tokens", "total_tokens")) for u in usages
        ) else None

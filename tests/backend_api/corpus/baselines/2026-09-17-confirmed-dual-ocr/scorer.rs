...
fn differences(actual: &Value, expected: &Value) -> Vec<String> {
    ...
    for (i, e) in wanted.iter().enumerate() {
        ...
        if let Some(cn) = e["standard_name"].as_str() {
            let constraint =
                json!({"name":cn,"visible_name_fragments":e["standard_name_visible_fragments"]});
            if !name_ok(a["standardName"].as_str().unwrap_or(""), &constraint) {
                errors.push(format!("line[{i}].chinese: {} != {cn}", a["standardName"]));
            }
        }
    }
    for (i, e) in wanted.iter().enumerate() {
...
fn confirmed_receipt_scoring_uses_saved_values_not_user_aliases_or_unknown_fields() {
    ...
    prediction["lines"][0]["standardName"] = json!("custom user alias");
        ...
        let report = match result {
                ...
                .and_then(|s| serde_json::from_str::<Value>(s).ok())
            {
                Some(prediction) => {
                    ...
                    json!({"id":c["id"],"prediction":prediction,"errors":errors,"parsing":response["receipt_parsing"],"usage":response["usage"]})
                }
                None => {

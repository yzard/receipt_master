...
python tests/backend_api/confirmed_snapshot.py --url http://127.0.0.1:5000 \
  --config playground/backend_api/config.toml --output build/vision/confirmed-snapshot \
...
python tests/backend_api/vision_reported.py --url http://127.0.0.1:5000 \
  --config playground/backend_api/config.toml --output-directory build/vision/reported
```

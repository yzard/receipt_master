...
pub fn product(line: &mut Value) {
    ...
    // OCR can omit the gap between the numeric SKU and an alphabetic name.
    // Keep a required gap for names starting with digits or punctuation.
    let re = Regex::new(r"^(?:([A-Za-z])\s+)?([0-9]{3,})(?:\s+(.+)|([A-Za-z].*))$").unwrap();
    if let Some(c) = re.captures(name) {
        let sku = c[2].to_owned();
        let name = c.get(3).or_else(|| c.get(4)).unwrap().as_str().to_owned();
